...
Dataset: BillSum (legislative bill summarization)
"""
...
# CRITICAL: Set CUDA environment variables BEFORE any CUDA operations
...
# CRITICAL: Reset CUDA at script start
if torch.cuda.is_available():
    ...
    try:
        ...
        gc.collect()
        print("✓ CUDA reset successful at script start")
...
BILLSUM_PATH = BASE_PATH / "BillSum_Experiments"

# Setup logging
log_dir = BILLSUM_PATH / "logs"
log_dir.mkdir(parents=True, exist_ok=True)

timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
log_file = log_dir / f"billsum_all_experiments_{timestamp}.log"
...
# Configuration - Optimized for 4GB GPU with BillSum (longer documents)
CONFIG = {
    "dataset": "billsum",
    "max_steps": 2000,
    "batch_size": 4,  # BillSum has longer documents
    "gradient_accumulation_steps": 4,
    "learning_rate": 3e-4,
    "max_source_length": 1024,  # Legislative bills are long
    "max_target_length": 256,  # Summaries are longer
    "save_steps": 500,
    "logging_steps": 50,
    "warmup_steps": 100,
    "weight_decay": 0.01,
...
def load_and_prepare_dataset():
    """Load BillSum dataset from local disk"""
    logger.info("Loading BillSum dataset from local disk...")
    ...
    try:
        ...
        logger.info(f"✅ Successfully loaded from: {local_path}")
    except Exception as e:
        logger.error(f"Failed to load from local disk: {e}")
...
def preprocess_function_seq2seq(examples, tokenizer, model_type):
    ...
    # BillSum has 'text' and 'summary' fields
...
def preprocess_function_causal(examples, tokenizer):
    ...
    for text, summary in zip(examples["text"], examples["summary"]):
        ...
        formatted = f"Summarize this bill:\n\n{text}\n\nSummary: {summary}"
...
def run_experiment(exp_config, train_dataset):
    ...
    exp_name = f"{exp_id}_{folder}_BillSum_{method}"
    ...
    try:
        # CRITICAL: Clear GPU before each experiment
        if torch.cuda.is_available():
            import gc
            try:
                for _ in range(3):
                    ...
                    torch.cuda.empty_cache()
                    torch.cuda.synchronize()
                logger.info("✓ GPU cleared before experiment start")
            except Exception as e:
                logger.warning(f"GPU pre-cleanup warning: {e}")
        
        # Create output directories
        model_folder = BILLSUM_PATH / folder
        results_dir = model_folder / "results"
        checkpoints_dir = model_folder / "checkpoints" / exp_name
        cache_dir = BILLSUM_PATH / "cache" / model_type
        ...
        if method == "QLoRA":
            ...
            logger.info("QLoRA: Using batch_size=2, gradient_accumulation=8 (long documents)")
        elif method == "Adapters":
            if model_type == "gpt2":
                batch_size = 2
                gradient_accumulation = 8
                ...
                logger.info("Adapters: Using batch_size=3, gradient_accumulation=5 (long documents)")
        else:
            # LoRA
            if model_type == "gpt2":
                batch_size = 2
                gradient_accumulation = 8
                logger.info("LoRA (GPT-2): Using batch_size=2, gradient_accumulation=8 (memory protection)")
            else:
        ...
        results = {
            ...
            "note": "Legislative bill summarization - Causal LM" if model_type == "gpt2" else "Legislative bill summarization - Seq2Seq"
        }
        ...
        try:
            ...
            if torch.cuda.is_available():
                for _ in range(3):
                    ...
                    torch.cuda.synchronize()
                    gc.collect()
                logger.info("✓ GPU cleaned up after failure")
...
def main():
    ...
    logger.info("="*80)
    logger.info("BILLSUM ALL 9 EXPERIMENTS - 2000 STEPS")
    logger.info("="*80)
    logger.info(f"Start time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    logger.info(f"Base path: {BILLSUM_PATH}")
    logger.info(f"Configuration: {CONFIG}")
    logger.info("")
    
    # CRITICAL: Reset CUDA before starting
    if torch.cuda.is_available():
        import gc
        try:
            torch.cuda.empty_cache()
            ...
            torch.cuda.reset_peak_memory_stats()
            gc.collect()
            logger.info("✓ CUDA initialized and cleared")
        except Exception as e:
    ...
    for i, exp_config in enumerate(EXPERIMENTS, 1):
        ...
        if is_experiment_completed(exp_config['exp_id'], BILLSUM_PATH):
            logger.info(f"Skipping {exp_config['exp_id']} - already completed")
            # Load existing results
            for folder in ["BART", "T5", "GPT2"]:
                result_file = BILLSUM_PATH / folder / "results" / f"{exp_config['exp_id']}_results.json"
                if result_file.exists():
                    with open(result_file, 'r') as f:
        ...
        if torch.cuda.is_available():
            ...
            try:
                ...
                logger.warning(f"⚠ GPU cleanup warning (non-critical): {e}")
                ...
                try:
                    ...
                    pass
                logger.info("Basic GPU cleanup completed despite error")
        
        # Longer delay between experiments to let GPU settle
    ...
    # Save master summary
    summary_file = BILLSUM_PATH / f"billsum_all_summary_{timestamp}.json"
