#!/usr/bin/env python3
"""
SAMSum All 9 Experiments - 2000 Steps
BART, T5, GPT-2 with LoRA, QLoRA, Adapters
Dataset: SAMSum (conversational dialogue summarization)
"""
...
SAMSUM_PATH = BASE_PATH / "SAMSum_Experiments"

# Setup logging
log_dir = SAMSUM_PATH / "logs"
log_dir.mkdir(parents=True, exist_ok=True)

timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
log_file = log_dir / f"samsum_all_experiments_{timestamp}.log"
...
# Configuration - Optimized for 4GB GPU with SAMSum
CONFIG = {
    "dataset": "samsum",
    "max_steps": 2000,
    "batch_size": 8,  # SAMSum has short dialogues
    "gradient_accumulation_steps": 2,
    "learning_rate": 3e-4,
    "max_source_length": 512,  # Dialogue length
...
def load_and_prepare_dataset():
    """Load SAMSum dataset from local disk"""
    logger.info("Loading SAMSum dataset from local disk...")
    
    # Load dataset from local disk
    from datasets import load_from_disk
    local_path = "E:/Pending Experiment data/local_datasets/samsum"
...
def preprocess_function_seq2seq(examples, tokenizer, model_type):
    ...
    # SAMSum has 'dialogue' and 'summary' fields
...
def run_experiment(exp_config, train_dataset):
    ...
    exp_name = f"{exp_id}_{folder}_SAMSum_{method}"
    ...
    try:
        ...
        if method == "QLoRA":
            ...
            if model_type == "gpt2":
                batch_size = 4
                gradient_accumulation = 4
                logger.info("LoRA (GPT-2): Using batch_size=4, gradient_accumulation=4 (memory protection)")
            else:
        ...
        results = {
            ...
            "note": "Conversational dialogue summarization - Causal LM" if model_type == "gpt2" else "Conversational dialogue summarization - Seq2Seq"
        }
...
def main():
    ...
    logger.info("="*80)
    logger.info("SAMSUM ALL 9 EXPERIMENTS - 2000 STEPS")
    logger.info("="*80)
    logger.info(f"Start time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    logger.info(f"Base path: {SAMSUM_PATH}")
    ...
    for i, exp_config in enumerate(EXPERIMENTS, 1):
        ...
        if is_experiment_completed(exp_config['exp_id'], SAMSUM_PATH):
            logger.info(f"Skipping {exp_config['exp_id']} - already completed")
            # Load existing results
            for folder in ["BART", "T5", "GPT2"]:
                result_file = SAMSUM_PATH / folder / "results" / f"{exp_config['exp_id']}_results.json"
                if result_file.exists():
                    with open(result_file, 'r') as f:
    ...
    # Save master summary
    summary_file = SAMSUM_PATH / f"samsum_all_summary_{timestamp}.json"
