#!/usr/bin/env python3
"""
Evaluate BillSum E26 - GPT-2 QLoRA
Generates ROUGE-1, ROUGE-2, ROUGE-L scores
Run this AFTER train_billsum_e26_fixed.py completes successfully
"""
...
print("=" * 70)
print("EVALUATION: BillSum E26 - GPT-2 QLoRA")
print("=" * 70)

CHECKPOINT_DIR = Path("E:/Pending Experiment data/BillSum_Experiments/GPT2/checkpoints/E26_QLoRA")
RESULTS_DIR    = Path("E:/Pending Experiment data/BillSum_Experiments/GPT2/results")
BASE_MODEL     = "gpt2"
MAX_LENGTH     = 512
NUM_TEST       = 25   # match training test set size
...
for i, example in enumerate(test_data):
    ...
    prompt = f"Summarize: {example['text']}\n\nSummary:"
    inputs = tokenizer(
        prompt,
        return_tensors="pt",
        truncation=True,
        max_length=MAX_LENGTH - 80,   # leave room for generated summary
    ).to(model.device)

    with torch.no_grad():
        output_ids = model.generate(
            **inputs,
            max_new_tokens=80,        # BillSum summaries avg ~200 tokens; 80 is reasonable
            num_beams=2,
            early_stopping=True,
...
eval_results = {
    ...
    "note": "Source truncated to 432 tokens (512 - 80 for generation headroom)"
}
...
print("=" * 70)
print("\nUpdate Experiment_Results.csv with these scores for BILLSUM_E26")
