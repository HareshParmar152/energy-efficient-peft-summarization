...
print("="*80)
print("TRAINING: BillSum E2 (BART QLoRA)")
print("="*80)
...
try:
    ...
    train_dataset = dataset["train"].select(range(300))  # Smaller for QLoRA
    test_dataset = dataset["test"].select(range(30))
    print(f"   Train samples: {len(train_dataset)}")
    print(f"   Test samples: {len(test_dataset)}")

    print("\n3. Loading BART model with 4-bit quantization...")
    ...
    results = {
        ...
        "gradient_accumulation_steps": 16,
        "learning_rate": 3e-4,
        "note": "Legislative bill summarization - BART QLoRA"
    }
    ...
    result_file = results_dir / "E2_results_FAILED.json"
    with open(result_file, 'w') as f:
        json.dump(results, f, indent=2)
