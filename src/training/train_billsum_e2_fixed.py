#!/usr/bin/env python3
"""
BillSum E2 - BART QLoRA (FIXED for 4GB VRAM)
Key fixes vs previous attempt:
  - max_source_length=512 (truncate long bills - avg 1813 tokens causes OOM)
  - batch_size=1, gradient_accumulation=16 (effective batch=16)
  - LoRA r=4, alpha=8 (smaller rank = less memory)
  - double_quant=True (saves extra ~0.4 bits/param)
  - Aggressive pre-run GPU cleanup
...
print("=" * 70)
print("BillSum E2 - BART QLoRA (Memory-Optimized for 4GB VRAM)")
print("=" * 70)
...
results_dir     = BASE_PATH / "BART" / "results"
checkpoints_dir = BASE_PATH / "BART" / "checkpoints" / "E2_QLoRA"
results_dir.mkdir(parents=True, exist_ok=True)
checkpoints_dir.mkdir(parents=True, exist_ok=True)

# Key memory settings
# BillSum avg source = 1813 tokens — we truncate to 512 to fit in 4GB VRAM
# This is a trade-off: some bill content is lost but training is possible
...
tracker = EmissionsTracker(
    project_name="E2_BillSum_BART_QLoRA",
...
try:
    ...
    # 300 train samples (same as original script) — enough for 2000 steps
    # with batch=1, grad_accum=16 → 300 samples * ~6.7 epochs = 2000 steps
    train_data = dataset["train"].select(range(300))
    test_data  = dataset["test"].select(range(50))
    print(f"      Train: {len(train_data)} | Test: {len(test_data)}")

    # ── 3. Tokenizer ──────────────────────────────────────────────────────────
    print("[3/8] Loading BART tokenizer...")
    ...
    print("[5/8] Loading BART with 4-bit QLoRA quantization...")
    ...
    # Required for QLoRA backward pass - ensures input embeddings have grad_fn
    ...
    training_args = Seq2SeqTrainingArguments(
        ...
        gradient_checkpointing=False,  # Disabled - conflicts with QLoRA grad_fn
        report_to="none",
    ...
    results = {
        "experiment_id": "BILLSUM_E2",
        "experiment_name": "E2_BART_QLoRA",
        "model": MODEL_NAME,
        "model_type": "bart",
        "method": "QLoRA",
        "dataset": "billsum",
        ...
        "lora_r": 4,
        "lora_alpha": 8,
        "note": "BillSum BART QLoRA - fixed for 4GB VRAM, source truncated to 512 tokens"
    }

    result_file = results_dir / "E2_QLoRA_results.json"
    ...
    print("  python evaluate_billsum_e2.py")
    ...
    fail_file = results_dir / "E2_QLoRA_results_FAILED.json"
