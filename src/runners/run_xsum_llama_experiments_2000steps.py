#!/usr/bin/env python3
"""
XSum GPT-2 Experiments - 2000 Steps
Uses Causal LM approach for decoder-only models (E25-E27)
...
log_file = log_dir / f"xsum_gpt2_experiments_{timestamp}.log"
...
EXPERIMENTS = [
    {"exp_id": "E25", "model": "gpt2-medium", "method": "LoRA", "folder": "GPT2"},
    {"exp_id": "E27", "model": "gpt2-medium", "method": "Adapters", "folder": "GPT2"},
    {"exp_id": "E26", "model": "gpt2-medium", "method": "QLoRA", "folder": "GPT2"},
]
...
CONFIG = {
    "dataset": "xsum",
    "max_steps": 2000,
    "batch_size": 8,  # Reduced from 24 - too large was causing slowdown
    "gradient_accumulation_steps": 2,  # Increased to maintain effective batch size
    "learning_rate": 2e-4,
    "max_length": 256,  # XSum articles are shorter
    "save_steps": 500,
    "logging_steps": 50,
    "warmup_steps": 100,
    "weight_decay": 0.01,
...
def preprocess_function_causal(examples, tokenizer):
    """Preprocess for causal LM with prompt format"""
    inputs = []
    ...
    model_inputs = tokenizer(
        ...
        truncation=True,
        padding="max_length",  # Changed to max_length for causal LM
    )
...
def run_experiment(exp_config, train_dataset):
    """Run a single GPT-2 experiment"""
    exp_id = exp_config["exp_id"]
    model_name = exp_config["model"]
    method = exp_config["method"]
    folder = exp_config["folder"]
    
    exp_name = f"{exp_id}_GPT2_XSum_{method}"
    ...
    try:
        ...
        cache_dir = XSUM_PATH / "cache" / "gpt2"
...
def main():
    ...
    logger.info("="*80)
    logger.info("XSUM GPT-2 EXPERIMENTS - 2000 STEPS (Causal LM Approach)")
    logger.info("="*80)
    ...
    for i, exp_config in enumerate(EXPERIMENTS, 1):
        ...
        if is_experiment_completed(exp_config['exp_id'], XSUM_PATH):
            ...
            result_file = XSUM_PATH / "GPT2" / "results" / f"{exp_config['exp_id']}_results.json"
            with open(result_file, 'r') as f:
    ...
    # Save master summary
    summary_file = XSUM_PATH / f"xsum_gpt2_summary_{timestamp}.json"
