#!/usr/bin/env python3
"""
Multi-News All 9 Experiments - 2000 Steps
BART, T5, GPT-2 with LoRA, QLoRA, Adapters
Dataset: Multi-News (multi-document summarization)
"""
...
MULTINEWS_PATH = BASE_PATH / "MultiNews_Experiments"

# Setup logging
log_dir = MULTINEWS_PATH / "logs"
log_dir.mkdir(parents=True, exist_ok=True)

timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
log_file = log_dir / f"multinews_all_experiments_{timestamp}.log"
...
# Configuration - Optimized for 4GB GPU with Multi-News
CONFIG = {
    "dataset": "multi_news",
    "max_steps": 2000,
    "batch_size": 4,  # Multi-News has very long documents (multiple articles)
    "gradient_accumulation_steps": 4,
    "learning_rate": 3e-4,
    "max_source_length": 1024,  # Longer for multi-document input
    "max_target_length": 256,  # Longer summaries
    "save_steps": 500,
    "logging_steps": 50,
    "warmup_steps": 100,
    "weight_decay": 0.01,
...
def load_and_prepare_dataset():
    ...
    try:
        ...
        logger.info("Loaded with trust_remote_code=True")
    except Exception as e1:
        logger.warning(f"Method 1 failed: {e1}")
        try:
            # Method 2: Try direct parquet loading
            dataset = load_dataset("parquet", data_files={
                "train": "hf://datasets/alexfabbri/multi_news/train-*.parquet",
                "validation": "hf://datasets/alexfabbri/multi_news/validation-*.parquet",
                "test": "hf://datasets/alexfabbri/multi_news/test-*.parquet"
            })
            logger.info("Loaded from parquet files")
        except Exception as e2:
            logger.error(f"Method 2 failed: {e2}")
            # Method 3: Try without any config
            dataset = load_dataset("alexfabbri/multi_news")
            logger.info("Loaded with default method")
...
def preprocess_function_seq2seq(examples, tokenizer, model_type):
    ...
    # Multi-News has 'document' (multiple articles) and 'summary' fields
...
def preprocess_function_causal(examples, tokenizer):
    ...
    for doc, summary in zip(examples["document"], examples["summary"]):
        ...
        text = f"Summarize the following articles:\n\n{doc}\n\nSummary: {summary}"
        inputs.append(text)
...
def run_experiment(exp_config, train_dataset):
    ...
    exp_name = f"{exp_id}_{folder}_MultiNews_{method}"
    ...
    try:
        ...
        # Adjust batch size for memory constraints (Multi-News has long documents)
        batch_size = CONFIG["batch_size"]
        gradient_accumulation = CONFIG["gradient_accumulation_steps"]
        
        if method == "QLoRA":
            batch_size = 2  # Very small for long documents
            gradient_accumulation = 8
            logger.info("QLoRA: Using batch_size=2, gradient_accumulation=8 (long documents)")
        elif method == "Adapters":
            if model_type == "gpt2":
                batch_size = 1  # Minimal for GPT-2 Adapters with long docs
                gradient_accumulation = 16
                logger.info("Adapters: Using batch_size=1, gradient_accumulation=16 (memory protection)")
            else:
                batch_size = 3
                gradient_accumulation = 5
                logger.info("Adapters: Using batch_size=3, gradient_accumulation=5")
        else:
        ...
        results = {
            ...
            "note": "Multi-document summarization - Causal LM" if model_type == "gpt2" else "Multi-document summarization - Seq2Seq"
        }
...
def main():
    ...
    logger.info("="*80)
    logger.info("MULTI-NEWS ALL 9 EXPERIMENTS - 2000 STEPS")
    logger.info("="*80)
    logger.info(f"Start time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    logger.info(f"Base path: {MULTINEWS_PATH}")
    ...
    for i, exp_config in enumerate(EXPERIMENTS, 1):
        ...
        if is_experiment_completed(exp_config['exp_id'], MULTINEWS_PATH):
            logger.info(f"Skipping {exp_config['exp_id']} - already completed")
            # Load existing results
            for folder in ["BART", "T5", "GPT2"]:
                result_file = MULTINEWS_PATH / folder / "results" / f"{exp_config['exp_id']}_results.json"
                if result_file.exists():
                    with open(result_file, 'r') as f:
    ...
    # Save master summary
    summary_file = MULTINEWS_PATH / f"multinews_all_summary_{timestamp}.json"
